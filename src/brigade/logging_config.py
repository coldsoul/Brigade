...
def redirect_fds_to_file(path: Path):
    """Redirect the process's real stdout file descriptor to *path*.
    ...
    dependency that writes directly to fd 1 rather than through Python's
    `sys.stdout`/`logging`. Restores the original fd on exit, including on an
    unhandled exception.

    stderr (fd 2) is deliberately left alone: Textual renders the dashboard to
    stderr, so redirecting it would draw the TUI into the file instead of the
    terminal.
    """
