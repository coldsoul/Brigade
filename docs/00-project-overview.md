...
- **Local machine only.** No remote hosts, no systemd, no multi-machine sync. All daemons run as async tasks inside one foreground process (`relay up`).
- **One Interpreter session per project directory.** A "new project" is just a new directory with its own `.relay/` state — never one long-running session juggling multiple projects.
...
├── .mcp.json                    # wires the harness to the Interpreter's relay MCP tools
└── AGENTS.md / CLAUDE.md        # Interpreter persona + rules
```

`.relay/mailboxes/` and `.relay/state.json` are gitignored. `.relay/ledger/`, `config.toml`, and `personas/` are committed.

## CLI shape

```
relay init [dir]     # scaffold .relay/ into a new or existing directory
relay up               # foreground process: all role workers as async tasks, live status
relay status            # what's in flight, inbox depths
relay down
```
